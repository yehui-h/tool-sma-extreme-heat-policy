...
const MAPBOX_SUGGEST_UNAVAILABLE_NOTICE =
  "Location suggestions are temporarily unavailable. Please try again.";
...
export function useLocationSuggest({
  ...
  const [draftLocationInput, setDraftLocationInput] =
    useState<string>(initialLocationInput);
  const [draftSelectedLocation, setDraftSelectedLocation] =
  ...
  useEffect(() => {
    ...
    const selectedLocationValue =
      draftSelectedLocation?.formattedLocation.trim() ?? "";
        ...
        try {
          ...
          setSuggestError(
            dedupedSuggestions.length === 0
              ? MAPBOX_NO_LOCATION_FOUND_NOTICE
              : null,
          );
        } catch (error) {
  ...
  const onLocationInputChange = (value: string) => {
    ...
    const selectedLocationValue =
      draftSelectedLocation?.formattedLocation ?? "";
  ...
  const onLocationOptionSubmit = (value: string) => {
    const selectedSuggestion = locationSuggestions.find(
      (suggestion) => suggestion.formattedLocation === value,
    );
